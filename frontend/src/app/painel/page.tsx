...
const MOCK_STATS = {
  totalPets: 12,         // TODO (Lucas): GET /pets?registeredById=:userId → .length
  pendingAdoptions: 5,   // TODO (Lucas): GET /adoptions/received → filtrar status PENDING
  approvedAdoptions: 8,  // TODO (Lucas): GET /adoptions/received → filtrar status APPROVED
  newRescues: 2,         // TODO (Lucas): GET /rescue-help-requests (quando backend implementar)
};

// Últimas solicitações para o resumo do overview
...
// Últimos resgates para o overview
      ...
      <div className="max-w-7xl mx-auto px-8 py-10 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            {
              label: 'Resgates novos',
              value: newRescues,
              icon: '🚨',
              href: '#resgates',
              highlight: newRescues > 0,
            },
          ].map((metric) => (
            <Link key={metric.label} href={metric.href} className="block group">
              <div
                className={`bg-white rounded-[24px] p-6 border shadow-sm hover:shadow-md transition-all ${
                  metric.highlight
                    ? 'border-[#F4C542] shadow-[#F4C542]/10'
                    : 'border-gray-100'
                }`}
              >
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link
                href="/painel/pets"
                className="text-sm font-bold text-[#3A5B4F] hover:text-[#2C4A3E] transition-colors"
              >
                Ver todos →
              </Link>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-[#F9F7F2]">
                ...
                <span className="text-xs font-bold bg-[#E8F0E6] text-[#2C4A3E] px-3 py-1 rounded-full">
                  Disponível
                </span>
              </div>
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-[#F9F7F2]">
                ...
                <span className="text-xs font-bold bg-[#F4C542]/20 text-[#2C4A3E] px-3 py-1 rounded-full">
                  Em análise
                </span>
              </div>
            </div>
          ...
          <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm p-8">
            ...
            <div className="space-y-3">
              {MOCK_RECENT_ADOPTIONS.map((adoption) => (
                <div
                  key={adoption.id}
                  className="flex items-center gap-4 p-4 rounded-2xl bg-[#F9F7F2]"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-[#2C4A3E] text-sm truncate">
                  ...
                  <span
                    className={`text-xs font-bold px-3 py-1 rounded-full flex-shrink-0 ${
                      adoption.status === 'pending'
                        ? 'bg-[#F4C542]/20 text-[#2C4A3E]'
                        : 'bg-[#E8F0E6] text-[#2C4A3E]'
                    }`}
                  >
        ...
        <div id="resgates" className="bg-white rounded-[32px] border border-gray-100 shadow-sm p-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-1">
                Solicitações de resgate recebidas pela plataforma
              </p>
            </div>
            {newRescues > 0 && (
          ...
          <div className="space-y-4">
            {MOCK_RECENT_RESCUES.map((rescue) => (
              <div
                key={rescue.id}
                className="flex items-center justify-between gap-6 p-5 rounded-2xl bg-[#F9F7F2]"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      Recebido há {rescue.receivedAt}
                    </p>
                  </div>
                </div>
                {/* TODO (Lucas): conectar com rota de análise de resgates quando backend implementar */}
                <button className="bg-[#3A5B4F] text-white font-bold px-5 py-2.5 rounded-2xl hover:bg-[#2C4A3E] transition-all text-sm flex-shrink-0">
                  Analisar →
                </button>
              </div>
            ))}
